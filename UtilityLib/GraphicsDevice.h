...
extern ID3D11PixelShader	*GD_CreatePixelShader(GraphicsDevice *pGD,
						const uint8_t	*pCodeBytes, int codeLen);
extern ID3D11ComputeShader	*GD_CreateComputeShader(GraphicsDevice *pGD,
						const uint8_t	*pCodeBytes, int codeLen);
extern ID3D11ShaderResourceView	*GD_CreateTexSRV(GraphicsDevice *pGD, ID3D11Resource *pRes, DXGI_FORMAT fmt);
...
extern void GD_CSSetShader(GraphicsDevice *pGD, ID3D11ComputeShader *pCS);
extern void GD_CSSetConstantBuffer(GraphicsDevice *pGD, int slot, ID3D11Buffer *pBuf);
extern void GD_CSSetUnorderedAccessViews(GraphicsDevice *pGD,
	UINT startSlot, UINT numUAVs, ID3D11UnorderedAccessView * const *ppUAVs,
